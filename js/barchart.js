class Barchart {
  ...
  renderVis() {
    ...
    vis.bars = vis.chart
      .selectAll(".bar")
      .data(vis.aggregatedData) //binding data to rectangles
      .enter()
      .append("rect")
      .attr("class", "bar")
      //.attr("x", (d) => vis.xScale(vis.xValue(d))) //Don't forget "vis." !
      .attr("x", (d) => vis.xScale(d.key)) // Set x position based on data"s key using x-scale
      .attr("y", (d) => vis.yScale(d.count)) // Set y position based on count using y-scale
